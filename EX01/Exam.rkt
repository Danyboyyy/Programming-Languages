...
(define (auxiliarTakeif lst n)
	(if (<= n 0)
		null
		(if (empty? lst)
			null
			(cons (car lst) (auxiliarTakeif (cdr lst) (- n 1)))
		)
	)
)

(define (exam.takeif lst n f)
	(auxiliarTakeif (filter f lst) n)
)
...
(define (auxiliarDepth x tree n)
	(if (empty? tree)
		0
		(if (equal? (value tree) x)
			n
			(max (auxiliarDepth x (left tree) (+ n 1)) (auxiliarDepth x (right tree) (+ n 1)))
		)
	)
)

(define (exam.depth x tree)
	(auxiliarDepth x tree 0)
)
...
(define (auxiliarSelect table cols)
	(if (empty? table)
		null
		(cons (filter (lambda (x) (map (lambda (y) (equal? x y)) cols)) (car table)) (auxiliarSelect (cdr table) cols))
	)
)


(define (exam.select table cols)
	(auxiliarSelect table cols)
)
