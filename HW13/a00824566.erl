...
firstn([], _) -> [];
firstn(_, 0) -> [];
firstn(Lst, X) -> [hd(Lst) | firstn(tl(Lst), X - 1)].
...
sum(Lst) -> 
	if
		is_list(hd(Lst)) -> sum(hd(Lst)) + sum(tl(Lst));
		true -> hd(Lst) + sum(tl(Lst))
	end.
...
even(Lst) ->
	if
		hd(Lst) rem 2 == 0 -> [hd(Lst) | even(tl(Lst))];
		true -> (even(tl(Lst)))
	end.
...
reverse(Lst) ->
	if
		is_list(hd(Lst)) -> reverse(tl(Lst)) ++ [reverse(hd(Lst))];
		true -> reverse(tl(Lst)) ++ [hd(Lst)]
	end.
...
maskedSum(Lst, Mask) -> 
	if
		hd(Mask) == true -> hd(Lst) + maskedSum(tl(Lst), tl(Mask));
		true -> maskedSum(tl(Lst), tl(Mask))
	end.

% === evaluate ===

evaluate([], _) -> 0;
evaluate(Lst, X) -> hd(Lst) * math:pow(X, length(Lst) - 1) + evaluate(tl(Lst), X).
