...
listor :: [Int] -> [Int] -> [Int]
listor [] [] = []
listor (1:x) (_:y) = 1 : listor x y
listor (_:x) (1:y) = 1 : listor x y
listor (_:x) (_:y) = 0 : listor x y

-- === multiples ===

multiples :: [Int] -> Int -> [Int]
multiples [] _ = []
multiples (h:t) x = if (rem h x) == 0
...
toBinaryString x = if x < 0 
    then error "Function \"toBinaryString\" is not defined for negative numbers." 
...
evaluate [] _ = 0
evaluate (c:p) x = c * x ^ (length p) + evaluate p x
